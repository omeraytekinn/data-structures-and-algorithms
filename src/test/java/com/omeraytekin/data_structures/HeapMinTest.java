...
public class HeapMinTest {
    ...
    @BeforeEach
    void setUp() {
        heap = new Heap<>(HeapType.MIN_HEAP, 5, 2);
    }

    @Test
    void testValidConstructor() {
        Heap<Integer> heap = new Heap<>(HeapType.MIN_HEAP, 10, 2);
    ...
    void testConstructorWithInvalidInitSize() {
        Exception exception = assertThrows(IllegalArgumentException.class, () -> new Heap<>(HeapType.MIN_HEAP, 0, 2));
        assertEquals(exception.getMessage(), "initSize must be greater than zero");
    }

    @Test
    void testConstructorWithInvalidGrowingFactor() {
        Exception exception = assertThrows(IllegalArgumentException.class, () -> new Heap<>(HeapType.MIN_HEAP, 10, 1));
    ...
    void testInsertHeapGrows() {
        Heap<Integer> heap = new Heap<>(HeapType.MIN_HEAP, 5, 2);
        for (int i = 0; i < 6; i++) {
            heap.insert(i);
        }
